...
class contenidoDAO extends UsuariosDAO
{
    ...
    public function getContenidos($pkID_proyectoM)
    {

        $query = "SELECT *, YEAR(fecha) AS anio FROM contenido
                WHERE estadoV = 1 AND fkID_proyecto_marco = " . $pkID_proyectoM;

        return $this->EjecutarConsulta($query);
    }

    public function getContenido($filtro, $pkID_proyectoM)
    {
        if ($filtro == "'Todos'") {
            $where_anio = '';
        } else {
    ...
    public function getContenidosId($pkID)
    {
