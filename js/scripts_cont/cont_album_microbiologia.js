$(function() {
    ...
    $("#btn_album_microbiologia").click(function() {
        console.log("hola")
        $("#lbl_form_album_microbiologia").html("Nuevo Album");
        $("#lbl_btn_actionalbum_microbiologia").html("Guardar <span class='glyphicon glyphicon-save'></span>");
        $("#btn_actionalbum_microbiologia").attr("data-action", "crear");
        $("#form_album_microbiologia")[0].reset(); 
        console.log($("#fkID_grupo").val());   
    });  

    $("#btn_nuevafoto").click(function() {
        $("#lbl_form_foto_microbiologia").html("Nuevas Fotos");
        $("#lbl_btn_actionfoto_microbiologia").html("Guardar <span class='glyphicon glyphicon-save'></span>");
        $("#btn_actionfoto_microbiologia").attr("data-action", "crear");
        $("#form_foto_microbiologia")[0].reset();
    });
    //Definir la acción del boton del formulario 
    $("#btn_actionalbum_microbiologia").click(function() {
    ...
    $("#btn_actionfoto_microbiologia").click(function() {
    ...
    $("[name*='edita_album']").click(function() {
        $("#lbl_form_album_microbiologia").html("Edita Album");
        $("#lbl_btn_actionalbum_microbiologia").html("Guardar Cambios <span class='glyphicon glyphicon-save'></span>");
        $("#btn_actionalbum_microbiologia").attr("data-action", "editar");
        $("#form_album_microbiologia")[0].reset();
    ...
    function crea_album() {
        ...
        micro = $("#fkID_microbiologia").val(); 
        nombre = $("#nombre_album").val();  
        fecha = $("#fecha_album").val();
        data="nombre_album="+nombre+"&fecha_album="+fecha+"&fkID_microbiologia="+micro+ "&tipo=inserta&nom_tabla=galeria_microbiologia"
        console.log(data)
            $.ajax({
                type: "GET",
    ...
    function crea_foto() {  
         var data = new FormData($("#form_foto_microbiologia")[0]);
    ...
    function edita_album() {
        ...
        console.log("ya vamos tres")
            $.ajax({
                type: "GET",
                url: '../controller/ajaxController12.php',
                data: "nombre_album="+nombre+"&fecha_album="+fecha+"&pkID="+id+"&tipo=actualizar&nom_tabla=galeria_microbiologia",
                success: function(r) {
                    console.log(r);
                    location.reload();
                }
    ...
    function carga_album(id_album) {
        ...
        $.ajax({
            url: '../controller/ajaxController12.php',
            data: "pkID=" + id_album + "&tipo=consultar&nom_tabla=galeria_microbiologia",
        }).done(function(data) {
    ...
    function elimina_album(id_album) {
        ...
        if (confirma == true) {
            ...
            $.ajax({
                url: '../controller/ajaxController12.php',
                data: "pkID=" + id_album + "&tipo=eliminar_logico&nom_tabla=galeria_microbiologia",
            }).done(function(data) {
                //---------------------
                console.log(data);
                location.reload();
    ...
    function elimina_foto(id_foto) {
        ...
        if (confirma == true) {
            ...
            $.ajax({
                url: '../controller/ajaxController12.php',
                data: "pkID=" + id_foto + "&tipo=eliminar_logico&nom_tabla=fotos_microbiologia",
            }).done(function(data) {
                console.log(data);
                location.reload();
            }).fail(function() {
    ...
    function validaEqualIdentifica(nombre) {
        ...
        var consEqual = "SELECT COUNT(*) as res_equal FROM galeria_microbiologia where estadoV= 1 and nombre_album='" + nombre + "'";
        $.ajax({
            url: '../controller/ajaxController12.php',
