$(function() {
    ...
    $("#btn_album_biotecnologia").click(function() {
        console.log("hola")
        $("#lbl_form_album_biotecnologia").html("Nuevo Album");
        $("#lbl_btn_actionalbum_biotecnologia").html("Guardar <span class='glyphicon glyphicon-save'></span>");
        $("#btn_actionalbum_biotecnologia").attr("data-action", "crear");
        $("#form_album_biotecnologia")[0].reset(); 
        console.log($("#fkID_grupo").val());   
    });  

    $("#btn_nuevafoto").click(function() {
        $("#lbl_form_foto_biotecnologia").html("Nuevas Fotos");
        $("#lbl_btn_actionfoto_biotecnologia").html("Guardar <span class='glyphicon glyphicon-save'></span>");
        $("#btn_actionfoto_biotecnologia").attr("data-action", "crear");
        $("#form_foto_biotecnologia")[0].reset();
    });
    //Definir la acción del boton del formulario 
    $("#btn_actionalbum_biotecnologia").click(function() {
    ...
    $("#btn_actionfoto_biotecnologia").click(function() {
    ...
    $("[name*='edita_album']").click(function() {
        $("#lbl_form_album_biotecnologia").html("Edita Album");
        $("#lbl_btn_actionalbum_biotecnologia").html("Guardar Cambios <span class='glyphicon glyphicon-save'></span>");
        $("#btn_actionalbum_biotecnologia").attr("data-action", "editar");
        $("#form_album_biotecnologia")[0].reset();
    ...
    function crea_album() {
        ...
        bio = $("#fkID_biotecnologia").val(); 
        nombre = $("#nombre_album").val();  
        fecha = $("#fecha_album").val();
        data="nombre_album="+nombre+"&fecha_album="+fecha+"&fkID_biotecnologia="+bio+ "&tipo=inserta&nom_tabla=galeria_biotecnologia"
        console.log(data)
            $.ajax({
                type: "GET",
    ...
    function crea_foto() {  
         var data = new FormData($("#form_foto_biotecnologia")[0]);
    ...
    function edita_album() {
        ...
        console.log("ya vamos tres")
            $.ajax({
                type: "GET",
                url: '../controller/ajaxController12.php',
                data: "nombre_album="+nombre+"&fecha_album="+fecha+"&pkID="+id+"&tipo=actualizar&nom_tabla=galeria_biotecnologia",
                success: function(r) {
                    console.log(r);
                    location.reload();
                }
    ...
    function carga_album(id_album) {
        ...
        $.ajax({
            url: '../controller/ajaxController12.php',
            data: "pkID=" + id_album + "&tipo=consultar&nom_tabla=galeria_biotecnologia",
        }).done(function(data) {
    ...
    function elimina_album(id_album) {
        ...
        if (confirma == true) {
            ...
            $.ajax({
                url: '../controller/ajaxController12.php',
                data: "pkID=" + id_album + "&tipo=eliminar_logico&nom_tabla=galeria_biotecnologia",
            }).done(function(data) {
                //---------------------
                console.log(data);
                location.reload();
    ...
    function elimina_foto(id_foto) {
        ...
        if (confirma == true) {
            ...
            $.ajax({
                url: '../controller/ajaxController12.php',
                data: "pkID=" + id_foto + "&tipo=eliminar_logico&nom_tabla=fotos_biotecnologia",
            }).done(function(data) {
                console.log(data);
                location.reload();
            }).fail(function() {
    ...
    function validaEqualIdentifica(nombre) {
        ...
        var consEqual = "SELECT COUNT(*) as res_equal FROM galeria_biotecnologia where estadoV= 1 and nombre_album='" + nombre + "'";
        $.ajax({
            url: '../controller/ajaxController12.php',
