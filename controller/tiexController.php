...
class tiexController extends tiexDAO
{
    ...
    public function getSelectEstudiantes()
    {
        ...
        echo '<select name="fkID_estudiante_tiex" id="fkID_estudiante_tiex" class="form-control" required = "true">
    ...
    public function getSelectAlbumTiex($pkID_tiex)
    {

        $album = $this->getAlbumTiex($pkID_tiex);

        if ($album[0]["pkID"]!="") {
            for ($a = 0; $a < sizeof($album); $a++) {
        $fotos = $this->getFotosTiex($album[$a]["pkID"]); 
        if ($fotos[0]["pkID"]=="") {
                 echo '<div class="col-md-2 text-center">
                                <button id="edita_album" title="Editar" name="edita_album" type="button" class="btn btn-warning" data-toggle="modal" data-target="#frm_modal_album_tiex" data-id-album = "' . $album[$a]["pkID"] . '" ';
                    echo '><span class="glyphicon glyphicon-pencil"></span></button>
                     ...
                     <a title="album" href="fotos_album_tiex.php?id_album='.$album[$a]["pkID"].'">
