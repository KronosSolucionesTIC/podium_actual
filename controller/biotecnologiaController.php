...
class biotecnologiaController extends biotecnologiaDAO
{
    ...
    public function getSelectAlbumBiotecnologia($pkID_biotecnologia)
    {

        $album = $this->getAlbumBiotecnologia($pkID_biotecnologia);

        if ($album[0]["pkID"]!="") {
            for ($a = 0; $a < sizeof($album); $a++) {
        $fotos = $this->getFotosBiotecnologia($album[$a]["pkID"]); 
        if ($fotos[0]["pkID"]=="") {
                 echo '<div class="col-md-2 text-center">
                                <button id="edita_album" title="Editar" name="edita_album" type="button" class="btn btn-warning" data-toggle="modal" data-target="#frm_modal_album_biotecnologia" data-id-album = "' . $album[$a]["pkID"] . '" ';
                    echo '><span class="glyphicon glyphicon-pencil"></span></button>
                     ...
                     <a title="album" href="fotos_album_biotecnologia.php?id_album='.$album[$a]["pkID"].'">
